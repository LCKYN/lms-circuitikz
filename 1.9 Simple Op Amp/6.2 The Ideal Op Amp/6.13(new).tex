\input{../../preamble.tex}
\begin{document}
\begin{circuitikz}[american,cute inductors]
    \input{../../preamble2.tex}

    \draw(0,0) node[ground]{}
        to[sVpmh,l=\vsname{1}] ++(0,3)
        to[short] ++(0,1)
        to[short] ++(2,0)
        to[R,l=\rname{1},-*] ++(3,0) node[op amp, noinv input down, anchor=-](oa){} (2,0) node[ground]{}
        to[sVpmh,l=\vsname{2}] ++(0,0 |- oa.+)
        to[R,l=\rname{2},-*] (oa.+)
        to[R,l_=\rname{3}] (oa.+ |- 0,0) node[ground]{} (oa.-)
        to[short] ++(0,1.5) coordinate(temp)
        to[R,l=\rname{4}] (oa.out |- temp)
        to[short,-*] (oa.out)
        to[short,-o] ++(1,0) node[magenta, anchor=west]{$v_o$}
        ;

    %%% find %%%
    % node[op amp, noinv input up, anchor=-](oa){}
    % node[anchor=west]{$a$}
    % \node at(3.7,4.6) []{$t=0$};
    % \node at(9,6) []{$v_o$};
    
    % \draw[magenta](15,9)  
    %     to[open,v^=$v_o$] ++(0,-3.5);

    % \draw[magenta](16.8,4.5)  
    %     to[open,v^=$v_o$] ++(0,-4.5);

    % \draw[circuitikz/current arrow color=magenta](5,5)
    % to[open,f>^=$\color{magenta}{i_o}$] ++(2,0);
\end{circuitikz}
\end{document}
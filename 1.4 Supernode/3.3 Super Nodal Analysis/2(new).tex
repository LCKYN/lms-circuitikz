...
\begin{circuitikz}[american voltages]
    ...
    \draw (2,0) to[R, l=$R_3$] ++(0,3) to[short] ++(0,2);
    \draw (5,0) to[R, l=$R_4$] ++(0,3);
    \draw (8,0) to[american current source, l=$I_{S2}$] ++(0,3) to[short] ++(0,2);
    
    % y
    \draw (0,0) to[short,-*] ++(2,0) to[short,-*] ++(3,0) node[ground]{} to[short] ++(3,0);
    \draw (8,3) to[R, l_=$R_2$] ++(-3,0) to[american voltage source, l_=$V_{S1}$] ++(-3,0) to[short] ++(-2,0);
    \draw (2,5) to[R, l=$R_1$] ++(6,0);

    % node
    % \draw[circuitikz/current arrow color=purple](14,0) to[R, l=$R_4$, f_<=$\color{purple}{i_4}$] ++(0,5);
    \draw(2,3) node[circ, color=purple]{} node[anchor=south west,color=purple] {$v_1$};
    \draw(5,3) node[circ, color=purple]{} node[anchor=south west,color=purple] {$v_2$};
    \draw(8,3) node[circ, color=purple]{} node[anchor=south west,color=purple] {$v_3$};

...
\begin{circuitikz}[american voltages]
    ...
    % y
    \draw (0,0) to[american current source, l=$I_{S1}$] ++(0,6);
    \draw (3,0) to[american controlled current source, l=$v_{x}$, *-] ++(0,3) to[R,l=$R_1$] ++(0,3);
    \draw[circuitikz/current arrow color=magenta] (6,6) to[R,l_=$R_3$,i=$\color{magenta}{i}$] ++(0,-3) to[short,*-] ++(0,-3);

    % x
    \draw(0,0) to[short] ++(6,0);
    \draw(0,6) to[short] ++(6,0);
    \draw(3,3) to[R,l=$R_2$] ++(3,0);
    \draw[color=magenta](3.5,3) to[open,v>=$v_x$] ++(2,0);
